 ...
 *³ Name      : Get/Set Funktionen fr globale Variablen                   ³
...
// fr folgende Settings werden keine extra Zugriffsfunktionen ben”tigt
// da Zugriffe ber die folgenden Funktionen implementiert sind
// DEMO               QueryDemo/EnableDemo
// ANIMATION          QueryAnimation/EnableAnimation
// POINTER            QueryCurrentSettings/LoadPointerAnimation

// fr folgende SETTINGS sind Zugriffsfunktionen definiert
...
ScanSetupStringAsync( PVOID Parms )
{
  ...
  // Priorit„t des Initialisierungsthreads setzen
  ...
  // ggfs. Initialisierung verz”gern
...
ScanSetupString( HWND hwnd, PRECORDCORE pcnrrec, PSZ pszSetup, BOOL fModify, BOOL fCallAsync )
{
  ...
  do
  {
    // Note: Parameterprfung sp„ter machen, damit
    ...
    // notifizieren, dass die initialisierung durchgefhrt wird
    // bzw. "bald fertig ist"
    ...
    while( *pszScan != 0 )
    {
      ...
      // jetzt prfen, ob das unser Schlsselwort ist
      for( i = 0; i < ulStringCount; i++ )
      {
        if( strncmp( pszScan, apszValidStrings[ i], strlen( apszValidStrings[ i] )) == 0 ) {
          ...
          // entsprechende Aktion durchfhren
          switch( i )
          {
            // Stati nur vormerken und sp„ter setzen !
            case 0:
              fToggleDemo = TRUE;
            ...
            case 3:
            {
              ...
              do
              {
                ...
                // zweiter Wert: Angabe fr alle oder undefinierte
                pszValue = pszNextValue;
                pszNextValue = strchr( pszValue, SEPVALUE );
                if( pszNextValue != NULL ) {
                  *pszNextValue = 0;
      ...
      // eigenen String l”schen, fremden String berspringen
      if( fOwnString ) {
    ...
    // Stati setzen, nur einen Refresh durchfhren
    ...
    // soll das Original ver„ndert werden ?
